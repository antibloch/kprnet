...
def main():
    ...
    point_fil_names = [path for path in point_fil_names if "points" in path]
    point_fil_names = sorted(point_fil_names, key=lambda x: int(x.split("_")[0]))

    pred_fil_names = os.listdir(args.predictions)
    pred_fil_names = [path for path in pred_fil_names if "points" in path]
    pred_fil_names = sorted(pred_fil_names, key=lambda x: int(x.split("_")[0]))
    ...
    for id, point_file, pred_file in zip(range(len(point_fil_names)), point_fil_names, pred_fil_names):
