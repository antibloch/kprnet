...
def train():
    ...
    model.load_state_dict(
        torch.load(
            args.model_dir / "resnext_cityscapes_2p.pth", map_location=f"cuda:0"
        ),
        strict=False,
    )
    # dist.barrier()
    ...
    for epoch in range(120):
        ...
        for step, items in enumerate(train_loader):
            ...
            px = items["px"].float().cuda(non_blocking=True).detach()

            pxyz = items["points_xyz"].float().cuda(non_blocking=True)
            knns = items["knns"].long().cuda(non_blocking=True)
